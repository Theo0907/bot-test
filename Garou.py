...
class Garou:
    ...
    async def gstart(self, ctx):
        ...
        if game == 1:
            ...
            print("Lancement d'une partie de Garou par: "+str(ctx.message.author))
            await self.bot.say("Lancement d'une partie !")
            await self.bot.say("Veuillez spécifier les joueurs avec la commande gjoueurs")
    ...
    async def gjoueurs(self, ctx, *, CMDjoueurs):
        ...
        if game == 0:
            ...
            if djoueurs == 1:
                print("Commande gjoueurs lancée par: "+str(ctx.message.author)+" refuser, partie deja lacer !")
                await slef.bot.say("Désoler mais une partie est deja en cours !")

            elif djoueurs == 0:
                djoueurs = 1
                joueurs = CMDjoueurs
                print("Commande gjoueurs lancée par: "+str(ctx.message.author)+" argument: "+str(CMDjoueurs))
